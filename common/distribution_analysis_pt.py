        ...
        for model in bkgModels:      
            ...
            for iBin in range(1, h1RawCounts.GetNbinsX() + 1):
                ...
                h1RawCounts.SetBinContent(iBin, h2RawCounts.GetBinContent(
                    iBin,1) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCounts.GetBinWidth(iBin) / (PT_BINS[iBin-1]+PT_BINS[iBin])/2 / n_ev)
                h1RawCounts.SetBinError(iBin, h2RawCounts.GetBinError(
                    iBin,1) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCounts.GetBinWidth(iBin) / (PT_BINS[iBin-1]+PT_BINS[iBin])/2 / math.sqrt(n_ev*n_run))
                
                h1RawCountsPt.SetBinContent(iBin, h2RawCounts.GetBinContent(
                    iBin,1) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCountsPt.GetBinWidth(iBin) / n_ev)
                h1RawCountsPt.SetBinError(iBin, h2RawCounts.GetBinError(
                    iBin,1) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCountsPt.GetBinWidth(iBin) / math.sqrt(n_ev*n_run))
                ...
                for eff in np.arange(ranges['SCAN'][iBin - 1][0], ranges['SCAN'][iBin - 1][1], ranges['SCAN'][iBin - 1][2]):
                    ...
                    raws[iBin-1].append(h2RawCounts.GetBinContent(iBin,1) / h1PreselEff.GetBinContent(iBin) / eff / h1RawCounts.GetBinWidth(iBin) / (PT_BINS[iBin-1]+PT_BINS[iBin])/2 / n_ev)
                    errs[iBin-1].append(h2RawCounts.GetBinError(iBin,1) / h1PreselEff.GetBinContent(iBin) / eff / h1RawCounts.GetBinWidth(iBin) / (PT_BINS[iBin-1]+PT_BINS[iBin])/2 / math.sqrt(n_ev*n_run) )
            ...
            cvDir.cd()
            myCv = TCanvas(f"mT_SpectraCv_{model}{split}")
            ...
            cvDir.cd()
            myCv = TCanvas(f"pT_SpectraCv_{model}{split}")
            gPad.SetLeftMargin(0.15); 
