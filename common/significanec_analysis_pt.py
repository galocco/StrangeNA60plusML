...
for sigmodel in SIG_MODELS:
    for bkgmodel in BKG_MODELS:
        ...
        for iBin in range(1, h1RawCounts[sigmodel][bkgmodel].GetNbinsX() + 1):
            ...
            h1RawCounts[sigmodel][bkgmodel].SetBinContent(iBin, h1Counts.GetBinContent(iBin) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / (MT_BINS_M[iBin]-MT_BINS_M[iBin-1])/ (MT_BINS_M[iBin-1]+MT_BINS_M[iBin])/2 / NEVENTS_DATA)
            h1RawCounts[sigmodel][bkgmodel].SetBinError(iBin, h1Counts.GetBinError(iBin) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / (MT_BINS_M[iBin]-MT_BINS_M[iBin-1])/ (MT_BINS_M[iBin-1]+MT_BINS_M[iBin])/2 / math.sqrt(NEVENTS_DATA*n_run))
            
            h1RawCountsPt[sigmodel][bkgmodel].SetBinContent(iBin, h1Counts.GetBinContent(iBin) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCountsPt[sigmodel][bkgmodel].GetBinWidth(iBin) / NEVENTS_DATA)
            h1RawCountsPt[sigmodel][bkgmodel].SetBinError(iBin, h1Counts.GetBinError(iBin) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCountsPt[sigmodel][bkgmodel].GetBinWidth(iBin) / math.sqrt(NEVENTS_DATA*n_run))
            ...
            for eff in np.arange(ranges['SCAN'][iBin - 1][0], ranges['SCAN'][iBin - 1][1], ranges['SCAN'][iBin - 1][2]):
                ...
                raws[iBin-1].append(h1Counts.GetBinContent(iBin) / h1PreselEff.GetBinContent(iBin) / eff / h1RawCountsPt[sigmodel][bkgmodel].GetBinWidth(iBin)/ NEVENTS_DATA)
                errs[iBin-1].append(h1Counts.GetBinError(iBin) / h1PreselEff.GetBinContent(iBin) / eff / h1RawCountsPt[sigmodel][bkgmodel].GetBinWidth(iBin)/ math.sqrt(NEVENTS_DATA*n_run) )
